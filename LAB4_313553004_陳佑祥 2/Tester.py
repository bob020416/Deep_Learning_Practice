...
class Test_model(VAE_Model):
    ...
    def eval(self):
        ...
        for idx, (img, label) in enumerate(tqdm(val_loader, ncols=80)):
            ...
            # Calculate PSNR for each frame in the sequence
            for frame_idx in range(len(pred_seq)):
                pred_frame = pred_seq[frame_idx].cpu().numpy().reshape(3, 32, 64)
                label_frame = label[0][frame_idx].cpu().numpy()
                psnr_value = self.calculate_psnr(pred_frame, label_frame)
                psnr_values.append(psnr_value)

        # Plot PSNR values
        self.plot_psnr(psnr_values, os.path.join(self.args.save_root, 'psnr_plot.png'))
    ...
    def plot_psnr(self, psnr_values, save_path):
        plt.figure()
        plt.plot(psnr_values, label='PSNR')
        plt.xlabel('Frame')
        plt.ylabel('PSNR (dB)')
        plt.title('PSNR during inference')
        plt.legend()
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        # TODO
        previous_frame = img[0].to(self.args.device)  #img 0 
        
        for i in range(1, len(label)):
            current_label = label[i].to(self.args.device)

            # Encode the current frame and label
            encoded_frame = self.frame_transformation(previous_frame)
            encoded_label = self.label_transformation(current_label)

            # Posterior prediction and reparameterization trick
            z, mu, logvar = self.Gaussian_Predictor(encoded_frame, encoded_label)
            
            # Decoder fusion and frame generation
            fusion_output = self.Decoder_Fusion(encoded_frame, encoded_label, z)
            predicted_frame = self.Generator(fusion_output)

            # Store the predicted frame and current label
...
# python Lab4_template/Tester.py --DR LAB4_Dataset/LAB4_Dataset --save_root Lab4_template/check_point --ckpt_path Lab4_template/check_point/kaggle_32.ckpt --device cuda
# Set Seed for reproduction kaggle test 42 = 32.56801
# 43 = 32.56206
# 44 = 32.56621
# 55 = 31.65325
# 40
def main(args):
    set_seed(42)  # Set your seed value here
...
# python Lab4_template/Tester.py --DR LAB4_Dataset/LAB4_Dataset --save_root Lab4_template/check_point --ckpt_path Lab4_template/check_point/best_34.ckpt --device cuda